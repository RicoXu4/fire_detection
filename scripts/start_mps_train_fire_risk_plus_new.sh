...
LOG=/Users/rico/code/fire_detection/runs/train/mps_fire_risk_plus_new.log
exec >> "$LOG" 2>&1

main() {
  echo "=== Fire risk single-class MPS training start: $(date) ==="
  ...
  if [[ ! -f datasets/mixed_indoor_fire_plus_new/data.yaml ]]; then
    ...
    echo "source dataset already exists: datasets/mixed_indoor_fire_plus_new/data.yaml"
  fi
...
model.train(
    ...
    device="mps",
    project="runs/train",
    name="yolo26n_fire_risk_plus_new_mps_30e",
    patience=10,
    cos_lr=True,
    close_mosaic=10,
    hsv_h=0.015,
